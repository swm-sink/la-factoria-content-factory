...
from app.models.export import (
    ExportJob, ExportStatus, ExportRequest,
    ExportJobResponse, ExportJobList
)
from app.models.pydantic.user import User
from app.schemas.export import (
    ExportRequestSchema, ExportJobResponseSchema,
    ExportProgressSchema, BulkExportRequestSchema
)
...
async def list_export_jobs(
    ...
    return ExportJobList(
        ...
        page=page,
        page_size=page_size,
        total_pages=jobs_data["total_pages"]
    )
...
async def cancel_export_job(
    ...
    if export_job.status in [ExportStatus.COMPLETED, ExportStatus.FAILED]:
        raise HTTPException(
            status_code=400,
            detail=f"Cannot cancel job with status: {export_job.status.value}"
        )
