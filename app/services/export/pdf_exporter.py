...
class PDFExporter(BaseExporter):
    ...
    async def export_single(
        ...
        if content.content_outline.target_audience or content.content_outline.difficulty_level:
            ...
            if content.content_outline.estimated_total_duration:
                metadata_data.append(["Estimated Duration:", f"{content.content_outline.estimated_total_duration} minutes"])
        ...
        for section in content.content_outline.sections:
            ...
            if section.estimated_duration_minutes:
                story.append(Paragraph(
                    f"<i>Duration: {section.estimated_duration_minutes} minutes</i>",
                    self.styles['Normal']
                ))
            story.append(Spacer(1, 0.2*inch))
        ...
        if content.study_guide:
            ...
            concepts_list = ListFlowable(
                [ListItem(Paragraph(concept, self.styles['Normal'])) 
    ...
    async def export_batch(
        ...
        for idx, content in enumerate(contents):
            ...
            # Add content number
            story.append(Paragraph(
                f"Content Item {idx + 1} of {len(contents)}",
                self.styles['Normal']
            ))
            story.append(Spacer(1, 0.2*inch))
