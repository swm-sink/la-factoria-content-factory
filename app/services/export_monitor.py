...
export_file_size_bytes = Summary(
    'export_file_size_bytes',
    'Size of exported files in bytes',
    ['format']
)

active_export_jobs = Gauge(
    'active_export_jobs',
...
export_content_items_total = Counter(
    'export_content_items_total',
    'Total number of content items exported',
    ['format']
)
...
class ExportMonitor:
    ...
    async def record_export_request(
        self,
        format: ExportFormat,
        content_count: int
    ):
        ...
        export_content_items_total.labels(
            format=format.value
        ).inc(content_count)
        
        self.logger.info(
            f"Export request recorded: format={format.value}, items={content_count}"
        )
    ...
    async def record_export_error(
        ...
        """
        export_errors_total.labels(
            format=format.value,
            error_type=error_type
        ).inc()
        
        self.logger.error(
            f"Export error recorded: format={format.value}, "
    ...
    async def get_user_export_summary(
        self,
        user_id: str,
        days: int = 30
    ) -> Dict[str, any]:
