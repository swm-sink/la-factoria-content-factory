"""
In-memory progress tracking service for monitoring long-form content generation.

Note: This service provides an in-memory view of job progress, primarily used
internally by content generation services. The authoritative and persistent
job status and progress are typically managed by `JobManager` and stored in
Firestore. Care should be taken to ensure consistency if both are used,
or this service might be refactored to report progress to a persistent store.
"""
...
COMPLETED_GENERATIONS = Counter(
    "completed_content_generations_total", "Total completed generations"
)
FAILED_GENERATIONS = Counter(
    "failed_content_generations_total", "Total failed generations"
)
    ...
    def get_active_jobs(self) -> List[GenerationProgress]:
        ...
        with self._lock:
            return [
                ...
                not in [GenerationStage.COMPLETED, GenerationStage.FAILED]
            ]
