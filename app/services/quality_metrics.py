...
QUALITY_EVALUATION_TIME = Histogram(
    'quality_evaluation_duration_seconds',
    'Time spent on quality evaluation'
)
...
class QualityMetricsService:
    ...
    def _calculate_structure_metrics(
        ...
        has_conclusion = any(
            keyword in content_lower[-200:]
            for keyword in ['conclusion', 'summary', 'in summary', 'to wrap up', 'finally']
        )
        ...
        avg_paragraph_length = (
            ...
            if paragraph_count > 0 else 0
        )
        
        # Check for transitions
        transition_words = [
            'however', 'furthermore', 'moreover', 'additionally', 'meanwhile',
            'consequently', 'therefore', 'thus', 'next', 'first', 'second',
    ...
    def _calculate_relevance_metrics(
        ...
        syllabus_words = set(
            word.lower() for word in re.findall(r'\b\w+\b', syllabus_text)
            if len(word) > 3
        )
        ...
        if syllabus_words:
            keyword_coverage = len(syllabus_words.intersection(content_words)) / len(syllabus_words)
        else:
        ...
        completeness_score = min(1.0, len(content.split()) / 500)  # Based on content length
    ...
    def _calculate_engagement_score(self, content: str, target_format: str) -> float:
        ...
        if target_format == 'podcast':
            ...
            if any(phrase in content_lower for phrase in ['you might', 'let\'s', 'imagine', 'think about']):
            ...
            if any(phrase in content_lower for phrase in ['step', 'how to', 'you can', 'try']):
                engagement_score += 0.2
            
            # Check for examples
            if any(phrase in content_lower for phrase in ['example', 'for instance', 'such as']):
                engagement_score += 0.1
        
        # General engagement factors
        if any(phrase in content_lower for phrase in ['interesting', 'important', 'key', 'crucial']):
            engagement_score += 0.1
