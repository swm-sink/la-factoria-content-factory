...
from app.models.export import ExportFormat, ExportStatus, ExportJob
from app.models.pydantic.content import (
    GeneratedContent, ContentOutline, OutlineSection, ContentResponse
)
...
class TestExportManager:
        ...
        with patch('app.services.export_manager.batch_get_documents') as mock_batch_get:
            with patch('app.services.export_manager.create_or_update_job_in_firestore') as mock_update:
                ...
                mock_batch_get.return_value = {
                    "content-1": {
                        "user_id": user_id,
                        **sample_content_response.model_dump()
                    }
                }
    ...
    async def test_get_export_file_success(self, export_manager):
        ...
        test_file_path = os.path.join(
            export_manager.temp_storage_path,
            user_id,
            f"export_{job_id}.json"
        )
        ...
        with patch('app.services.export_manager.get_job_from_firestore') as mock_get_job:
            ...
            # Get export file
            content, content_type, filename = await export_manager.get_export_file(
                job_id=job_id,
                user_id=user_id
            )
    ...
    async def test_get_export_file_not_found(self, export_manager):
        ...
        with patch('app.services.export_manager.get_job_from_firestore') as mock_get_job:
            ...
            with pytest.raises(FileNotFoundError):
                await export_manager.get_export_file(
                    job_id=job_id,
                    user_id=user_id
                )
